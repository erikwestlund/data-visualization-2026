...
Modules are the course-owned notebooks we will use during class. I will often open a module, make the text large, talk through the setup, run code chunks step by step, and show the output as we go.

The `modules/` folder contains Quarto notebook source files:
...
- `modules/`: course-owned notebooks we can open and run during class
- `modules/rendered/`: HTML versions for quick viewing

Do not use `modules/` for your own saved work. Course updates may add, rename, or remove files in `modules/`.
...
- gets the latest instructor-owned course files if you are using the Git version of the course folder
